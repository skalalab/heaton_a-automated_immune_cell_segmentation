...
#%%
    df = pd.read_csv(r"C:\Users\abarres\Documents\GitHub\heaton_a-mouse_immune_cell_segmentation\Classification_Data\CD4_Mouse_Paper\ImmuneCellsvsTumorCells\CD4MousePaper_ImmunevsTumorCells_TME_OMI.csv")
    ...
    y = df["Cell"]
    df_data = df[list_features]

    #%% Scale data
    df_data_scaled = pd.DataFrame(
        StandardScaler().fit_transform(df_data), columns=list_features
    )

    #%% Split into test/train
    #dic_labels = {0: "...Control...", 1: "...Treated..."}
    list_labels = []
    for label in y:
        if "Tumor" in label:
    ...
    x_train, x_test, y_train, y_test = train_test_split(
        df_data_scaled.values, list_labels, train_size=0.7
    )
    #%% RANDOM FOREST
    ...
    print(
        f"Accuracy on the test set with raw data: {rf_random.score(x_test, y_test):.5f}"
    )
    ...
    if type(array_importances) != list : 
        array_importances = [array_importances]
    plt.figure(figsize=(10, 10))
    for feature, importance in zip(list_features, array_importances):
        pass
    ...
    plt.plot(
        ...
        tprSV,
        "r",
        label=f"Linear SVM, AUC:{roc_auc_score(y_test, SVraw_decision_scores):.5f}",
    )
    plt.plot(
        fprRF,
        tprRF,
        "g",
        label=f"Random Forest, AUC:{roc_auc_score(y_test, rf_raw_decision_scores[:,1]):.5f}",
    )
    plt.plot(
        fprLR,
        tprLR,
        "y",
        label=f"Logistic Regression, AUC:{roc_auc_score(y_test, lr_raw_decision_scores):.5f}",
    )
